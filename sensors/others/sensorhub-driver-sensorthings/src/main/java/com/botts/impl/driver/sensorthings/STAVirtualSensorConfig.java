...
public class STAVirtualSensorConfig extends SensorConfig {
    ...
    @DisplayInfo(desc = "MQTT Config", label = "MQTT config for streaming data. If MQTT is not used, the driver will default to HTTP polling.")
    public StreamConfig mqttConfig;

    @DisplayInfo(desc = "HTTP Poll Rate (ms)", label = "Rate (in milliseconds) to poll observations from service")
    public long httpPollRate = 1000;
